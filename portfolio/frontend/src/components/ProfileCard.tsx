      ...
      <div className="border border-white/50 rounded-3xl p-8">
        <div>
          <div className="flex justify-between items-center">
            <h1 className="w-full uppercase text-4xl ">{data?.ad_soyad}</h1>
            <h1 className="w-full text-start text-sm">{data?.unvan}</h1>
          </div>
          <div className="mt-8">
            <img
              src={"http://127.0.0.1:3333" + data?.resim}
              alt="resim"
              className="rounded-2xl grayscale-[60%]"
            />
          </div>
          <div className="text-center">
            <h2 className="text-xl mt-8 mb-4">{data?.mail}</h2>
            <h2 className="text-md">{data?.adres}</h2>
